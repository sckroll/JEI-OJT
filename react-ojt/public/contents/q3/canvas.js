...
const renderRandomShape = (canvas, shapeCount, minTriangleCount = 1) => {
  ...
  for (let i = 0; i < shapeCount; i++) {
    ...
    if (randomNumber <= 1) {
      // 삼각형 (0) & 패스 (1)
      const x1 = (~~(Math.random() * 2) === 0 ? 1 : -1) * (~~(Math.random() * 20) + 20)
      const y1 = (~~(Math.random() * 2) === 0 ? 1 : -1) * (~~(Math.random() * 20) + 20)
      const x2 = Math.cos(Math.atan2(y1, x1) + deg2Rad(~~(Math.random() * 60) + 60)) * (~~(Math.random() * 20) + 20)
      const y2 = Math.sin(Math.atan2(y1, x1) + deg2Rad(~~(Math.random() * 60) + 60)) * (~~(Math.random() * 20) + 20)
      ...
      const x1 = (~~(Math.random() * 2) === 0 ? 1 : -1) * (~~(Math.random() * 20) + 20)
      const y1 = (~~(Math.random() * 2) === 0 ? 1 : -1) * (~~(Math.random() * 20) + 20)
      const x2 = Math.cos(Math.atan2(y1, x1) + deg2Rad(~~(Math.random() * 70) + 30)) * (~~(Math.random() * 20) + 20)
      const y2 = Math.sin(Math.atan2(y1, x1) + deg2Rad(~~(Math.random() * 70) + 30)) * (~~(Math.random() * 20) + 20)
      const x3 = Math.cos(Math.atan2(y2, x2) + deg2Rad(~~(Math.random() * 70) + 30)) * (~~(Math.random() * 20) + 20)
      const y3 = Math.sin(Math.atan2(y2, x2) + deg2Rad(~~(Math.random() * 70) + 30)) * (~~(Math.random() * 20) + 20)
...
const renderModal = (canvas, titleText, buttonText, onClick) => {
  ...
  //   hoverCursor: 'default'
  // })
  const modalGroup = new fabric.Group([overlay, modalContainer, modalTitle], {
    top: 0,
    left: 0,
    opacity: 0,
    selectable: false,
...
export const initCanvas = () => {
    ...
    for (let i = 0; i < shapes.length; i++) {
      const shapeClickHandler = ({ target, e }) => {
        if (target.isTriangle) {
          ...
          correctCount++
          target.animate({
            top: 30, left: canvas.width / 4 + correctCount * 25, scaleX: 0.5, scaleY: 0.5, opacity: 1
          }, {
            onChange() {
              canvas.renderAll()
            },
  ...
  // renderLevel1(renderLevel2, () => renderModalWithTTS(messages.TITLE_MODAL_INCORRECT))
  renderLevel2(null, () => {
    renderModalWithTTS(messages.TITLE_MODAL_INCORRECT)
    setTimeout(() => {
